...
public class SearchActivity extends AppActivity {
    ...
    protected ArrayList<SearchOutput> results = new ArrayList<SearchOutput>();
    ...
    protected void onRestoreInstanceState(Bundle savedInstanceState) {
		...
		ArrayList<SearchOutput> searchResults = (ArrayList<SearchOutput>) savedInstanceState.getSerializable("searchResults");
    ...
    private class SearchTask extends AsyncTask<String, Object, ArrayList<SearchOutput>> implements DBListener{

        @Override
        protected ArrayList<SearchOutput> doInBackground(String... urls) {
            Log.d(TAG, "Starting search...");
            DbHelper db = new DbHelper(SearchActivity.this);
            String userName = prefs.getString(PrefsActivity.PREF_USER_NAME, "");
            ArrayList<SearchOutput> searchResults = db.search(currentSearch, 100, userId, SearchActivity.this, this, userName, isOnlyClientSearch);
            DatabaseManager.getInstance().closeDatabase();
        ...
        @Override
        protected void onPostExecute(ArrayList<SearchOutput> searchResults) {
            results.clear();
            results.addAll(searchResults);
            srla.notifyDataSetChanged();
